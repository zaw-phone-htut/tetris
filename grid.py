...
class Grid:
    ...
    def __init__(self):
        ...
        self.num_columns = 10
        self.cell_size = 30
        self.grid = [[0 for i in range(self.num_columns)]
    ...
    def is_inside(self, rows, columns):
        if rows >= 0 and rows < self.num_rows and columns >= 0 and columns < self.num_columns:
            return True
        return False
    ...
    def clear_full_row(self):
        completed_rows = 0
        # Starts from last row, goes up one by one
